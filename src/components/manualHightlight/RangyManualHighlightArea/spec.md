...
| 機能カテゴリ     | 詳細                               | 動作                                                                                  |
| ---------------- | ---------------------------------- | ------------------------------------------------------------------------------------- |
| **範囲選択**     | マウスドラッグによる自動ハイライト | テキスト選択 → 自動で赤い下線適用。ドキュメントに HTML タグが含まれていても適用可能。 |
| **範囲管理**     | 複数範囲の同時表示・保存           | 選択順序、テキスト、位置情報を保存                                                    |
| **一覧表示**     | モーダルダイアログでの選択履歴表示 | 順番・選択テキスト・シリアライズデータ・作成日時                                      |
| **削除機能**     | 個別削除・全削除                   | 特定範囲の削除またはすべてクリア                                                      |
| **スタイリング** | ドキュメントのスタイリング         | ハイライト対象ドキュメントのスタイリングをカスタマイズ可能                            |
...
| 対応範囲               | 具体例                                     | 説明                                              |
| ---------------------- | ------------------------------------------ | ------------------------------------------------- |
| **ブロックタグ内**     | `<p>科学技術の<strong>発展</strong></p>`   | `<p>`タグ内の複数インライン要素をまたいで選択可能 |
| **インラインタグ跨ぎ** | `<em>教育</em>、<mark>環境保護</mark>`     | 異なるインラインタグの境界を越えた選択            |
| **階層構造対応**       | `<ul><li><span>AI</span>の発達</li></ul>`  | ネストした要素構造でも正確にハイライト            |
| **混在コンテンツ**     | `テキスト<br/>改行後<sup>上付き</sup>文字` | テキストノードとタグが混在していても対応          |
...
本アプリの核心となる要素。リッチな HTML ドキュメント（マークアップされた文書、技術資料、学術論文など）に対して、安定したハイライト機能を提供するために利用。

- **ブラウザ標準の Selection API**では複雑な DOM 構造での範囲管理が困難
- **Rangy**は独自のアルゴリズムで DOM 要素の境界を正確に処理
- **シリアライズ機能**により、複雑な HTML 構造でも位置情報を正確に保存・復元
...
| **選択範囲のイベント** | 設定不可能                                                                                             |
| **スタイル**           | 固定（赤い 3px 下線）                                                                                  |
| **範囲重複**           | 重複範囲の選択は可能。ただし、前方の範囲から、後方の範囲と重複した場合、範囲選択失敗になる場合がある。 |
...
interface RangyManualHighlightAreaProps {
  ...
  onRangeSelect?: (range: SavedRange) => void; // 範囲選択時のコールバック。永続処理等に利用想定
  onRangeDelete?: (id: number) => void; // 範囲削除時のコールバック。永続処理等に利用想定
}
```
...
interface SavedRange {
  id: number; // 一意識別子。作成日時（タイムスタンプ）による簡易実装
  order: number; // 選択順序
